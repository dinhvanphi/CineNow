...
class _RoomListScreenState extends State<RoomListScreen> {
  ...
  Future<void> _loadCinemaData() async {
    try {
      ...
      if (cinemaId != null) {
        final response = await http.get(Uri.parse('http://localhost:3000/api/cinemas/$cinemaId'));
  ...
  Future<void> _loadMovieData() async {
    try {
      ...
      if (movieId != null) {
        final response = await http.get(Uri.parse('http://localhost:3000/api/movies/$movieId'));
        if (response.statusCode == 200) {
          final data = jsonDecode(response.body);
          setState(() {
